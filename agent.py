...
def _load_once():
    ...
    if not os.path.exists(wpath):
        raise FileNotFoundError(
            "waeights_new.pth not found next to agent.py. Train offline and include it in submission."
        )
...
# PRE-EMPTIVE WALL AVOIDANCE
    ...
    # ------------------------------------------------
# NO-SENSOR EXPLORATION FIX
# ------------------------------------------------

    if np.sum(obs[:17]) == 0:
     _last_action = ACTIONS.index("FW")
     _repeat_count = 0
     return "FW"
    # ------------------------------------------------
    # NORMAL POLICY
